...
| `validace_pam(data, labels_clean, path_out, id_cols, max_rok, compare_data, path_control)` | Main orchestrator; calls all of the above and returns the report list. `max_rok` filters data to `rok <= max_rok` before processing (default: current year − 1; `NULL` skips filtering). If `compare_data=TRUE` and `path_control` is set, compares output report with control (previous) run via `porovnej_pam_reporty()`. |
| `porovnej_pam_reporty(path_control_xlsx, path_test_xlsx)` | Compares two validation reports (xlsx files) line by line. Returns list with `$overview_compare` (structural metric diffs), `$summary_compare` (per-variable metric diffs), `$count_neshod` (count summary). Compares only shared columns to avoid spurious "column X missing in test" reports. |
| `porovnej_pam_varky(path_test, path_control)` | Batch comparison of two validation folders. Pairs xlsx files by stem (filename without `_validace_YYMMDD`). Returns list with `$overview_compare`, `$summary_compare`, `$count_neshod` (aggregated diffs across all matched reports), and `$missing_vykaz` (reports in test but not in control, or vice versa). |
| `format_summary_compare_wide(summary_compare)` | Converts long-format `summary_compare` to wide format with abbreviated change labels: numeric diffs show `vyšší *` / `nižší **` / `vyšší ***` based on % change (< 5%, 5–20%, > 20%); logical show `TRUE → FALSE`; text shows `old → new`; unchanged show `(stejný)`. |
| `write_pam_compare_xlsx(compare, path_out)` | Writes compare results to Excel report with two sheets: Overview (overview metrics + missing_vykaz) and Summary (wide-format per-variable diffs with abbreviated labels). Automatically called by `validace_PAM.R` when control folder exists. |
