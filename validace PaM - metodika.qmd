...
+----------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Čištění cyklických nul**       | Zda byla detekce cyklických nul povolena ve funkci `validace_pam()`                                                                                                                                                                                                                            |
+----------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `polozka_index_suffix`       | Unikátní kód každé proměnné včetně suffixu výkazu (např. `r102` z P1-04 se mapuje na `r01002.p1`, `r010102` z P1a na `r010102.1a`, `r01` z P1c-IV na `r04001.1c` atd.).                                                                                                                                                          |
+------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
### Mapování labelů na základě `polozka_index` {#sec-mapovani-labelu}

Jednotlivé r-proměnné se v parquet souborech a v tabulce `pamvykpol_labels.xlsx` identifikují pomocí kódů (např. `r102`, `r01`, `r010102` atd.), které se liší:
...
2.  **Suffixem** – parquet soubory obsahují `polozka_index` se suffixem výkazu (např. `r01.P1C`, `r010102.P1A`). Naproti tomu tabulka `pamvykpol_labels.xlsx` obsahuje ve sloupci `polozka` normalizované kódy bez suffixu, případně také sloupec `polozka_index` včetně suffixu.
...
2.  **Vytvoření jednoznačného identifikátoru proměnné napříč výkazy**
    ...
    Pro samotné mapování labelů je ve funkci `summarise_pam_variables()` vytvořena pomocná proměnná `polozka_index_suffix`, která převádí identifikátory proměnných do formátu používaného v tabulce `pamvykpol_labels.xlsx`, včetně suffixů označujících typ výkazu (např. `.p1`, `.1a`, `.1b`, `.1c`).

    Rekonstrukce se liší podle typu výkazu:

    -   **P1-04** – kód proměnné je doplněn na čtyřmístné označení pomocí úvodních nul před oddílem a řádky a přidán suffix `.p1`.\
        *např.* `r101` → `r01001.p1`

    -   **P1a** – původní kód proměnné je ponechán a pouze je doplněn suffix `.1a`.\
        *např.* `r010102` → `r010102.1a`

    -   **P1c** – kód je vytvořen z označení oddílu a řádku výkazu. Prefix odpovídá oddílu (`04`, `4a`, `4b`, `4c`, `06`, `08`), před číslo řádku je případně doplněna úvodní nula a na závěr přidán suffix `.1c`.\
        *např.* `r01` v oddíle IV → `r04001.1c`, `r01` v oddíle IVa → `r4a001.1c`.\
        Výjimkou je oddíl VI, kde označení oddílu vstupuje přímo do kódu proměnné, např. `r601` → `r06001.1c`.

3.  **Mapování labelů**

    Proměnná `polozka_index_suffix` je následně použita pro `match()` se sloupcem `polozka_index` v tabulce `pamvykpol_labels.xlsx`. Na základě nalezené shody je ke každé proměnné přiřazen odpovídající `label_result`, tj. slovní pojmenování proměnné.
...
Zároveň ještě před samotnou validací probíhá detekce cyklických nul. Kontroluje se, zda některý měsíc napříč všemi roky obsahuje pouze nulové hodnoty. V takovém případě se předpokládá, že se daná proměnná v tomto měsíci reálně neměří a do dat se pouze technicky přepisují nuly. Tyto cyklické nuly jsou proto funkcí `clean_cyclic_zeros()` převedeny na `NA`, aby bylo následně možné správně určit frekvenci měření a spočítat všechny potřebné statistiky.
{{< include docs/metodika-frekvence.qmd >}}
