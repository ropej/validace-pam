...
2. **Suffixem** – parquet soubory obsahují `polozka_index` se suffixem výkazu (např. `r001.p1c`, `r1.1a`), zatímco `pamvykpol_labels.xlsx` má sloupec `polozka_index` s normalizovanými kódy bez těchto suffixů

**Proces mapování:**

1. V `validace_PAM.R` se pomocí `rename_with()` **smažou suffixy** `.P*` (`.P04`, `.P1A`, `.P1C` apod.) – tím se normalizují kódy na sjednocenou formu
2. V `summarise_pam_variables()` se vytváří `polozka_index_suffix` – rekonstruovaný kód s suffixem výkazu, který jednoznačně mapuje na řádek v `pamvykpol_labels.xlsx`:
   - **P1-04:** `r001` → `r0001.p1` (čtyřmístný kód proměnné s prefixem `0` + suffix `.p1`)
   - **P1a:** `r1` → `r1.1a` (původní kód + suffix `.1a`)
   - **P1c:** `r1` → `r101.1c` / `r202.1c` (kód s prefixem oddílu podle indexu: IVa=1, IVb=2, atd., + suffix `.1c`)

3. Pomocí `polozka_index_suffix` se pak provádí `match()` se sloupcem `polozka` v `pamvykpol_labels.xlsx` a načítá se příslušný `label_result`

Tento mechanismus umožňuje, aby stejná r-proměnná (např. `r1`) mohla v různých výkazech a oddílech měla různé popisné štítky.
