...
|              | `r5s0802`–`r5s1305`        |                        |                                     |
+--------------+----------------------------+------------------------+-------------------------------------+
| **P1c**      | `r01`–`r14`, `r601`–`r612` | `ico` nebo `red_izo`   | více `.parquet` souborů dle oddílů\ |
|              |                            |                        | (IV, IVa, IVb, …, VIII)             |
+--------------+----------------------------+------------------------+-------------------------------------+
...
+------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `polozka_index_suffix`       | Unikátní kód každé proměnné včetně suffixu výkazu (např. `r001` z P1-04 se mapuje na `r0001.p1`, `r01001` z P1a na `r01001.1a`, `r01` z P1c-IV na `r04001.1c` atd.).                                                                                                                                                             |
+------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Jednotlivé r-proměnné se v parquet souborech a v tabulce `pamvykpol_labels.xlsx` identifikují pomocí kódů (např. `r001`, `r1`, `r01001` atd.), které se liší:

1.  **Délkou** – P1-04 má kódy 4-6 znaků (např. `r001`, `r01001`), P1a 2-6 znaků (např. `r1`, `r01001`), P1c 2-3 znaky (např. `r1`, `r2`)
2.  **Suffixem** – parquet soubory obsahují `polozka_index` se suffixem výkazu (např. `r001.p1c`, `r01001.1a`). Naproti tomu tabulka `pamvykpol_labels.xlsx` obsahuje ve sloupci `polozka` normalizované kódy bez suffixu, případně také sloupec `polozka_index` včetně suffixu.

Při mapování labelů proto bylo nutné sjednotit značení proměnných podle konkrétního zdroje a typu výkazu. Zohledněny byly zejména rozdíly v délce kódů mezi výkazy P1-04, P1a a P1c a také přítomnost či nepřítomnost suffixu výkazu.
...
1.  **Normalizace názvů proměnných**
    ...
    Ve skriptu `validace_PAM.R` jsou pomocí funkce `rename_with()` z názvů proměnných odstraněny suffixy `\\.*$` označující typ výkazu (např. `.P1`, `.1A`, `.1C`). Tím se sjednotí názvy proměnných napříč jednotlivými parquet soubory.

2.  **Vytvoření jednoznačného identifikátoru proměnné napříč výkazy**

    Ve funkci `summarise_pam_variables()` je vytvořena pomocná proměnná `polozka_index_suffix`, která rekonstruuje označení proměnné ve stejném formátu, jaký používá tabulka `pamvykpol_labels.xlsx`.
